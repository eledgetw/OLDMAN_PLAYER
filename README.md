...
* **Precision Looping**: Drag or use **Shift + Click** to select a range and loop it infinitely until your muscle memory takes over.
...
* **Variable Speed Control**: Slow down the action from **0.1x to 4.0x** without changing the pitch, with a visual toast indicator for quick feedback.
...
* **Draggable Region Management**: Saved practice regions (purple labels) can be **reordered via drag-and-drop** to match your practice flow.
* **Undo/Redo System**: Full state history (**Ctrl+Z / Ctrl+Y**) for all editing actions, including splitting, moving, and deleting clips.
* **Dynamic & Resizable UI**: Drag the separator to adjust the height of the timeline area; use **Cmd/Ctrl + Mouse Wheel** to zoom into specific notes.
...
* **Bilingual UI**: Full support for both **Traditional Chinese** and **English**.
...
* **精準循環播放**：透過拖曳或 **Shift + 點擊** 快速框選範圍，並無限循環播放，直到您的肌肉記憶徹底掌握。
* **視覺化節奏感**：播放器會渲染高解析度的音訊波形（最高達 15,000 個採樣點），具備**峰值顏色漸變**，讓您在彈奏前先「看見」節拍與樂句。
* **精確片段合併**：可將多個選取的碎片段無縫合併為一個完整的區塊，方便練習不同樂句間的銜接。
* **變速不變調**：支援 **0.1x 至 4.0x** 的速度調整，且提供大字體視覺提示（Toast），確保調整即時觸達。

## 🚀 核心功能
* **虛擬播放引擎**：支援「非連續性」時間軸播放。當片段之間有空隙時，播放器會自動黑屏並靜音，模擬專業剪輯軟體的運作邏輯。
* **物理防撞拖拽**：在 A 模式（選取模式）下移動片段時，具備自動防撞偵測，確保片段不會在同一軌道上重疊。
* **可拖拽區間管理**：下方自定義區間（紫色標籤）支援**滑鼠拖拽排序**，方便您依照練習順序重新排列樂句。
* **完善的復原系統**：支援全域 Undo/Redo（**Ctrl+Z / Ctrl+Y**），無論是剪錯位置還是不小心刪除片段都能瞬間救回。
* **可自訂 UI 佈局**：可自由拖動分隔線調整下方編輯區高度，並支援 **Cmd/Ctrl + 輪** 快速縮放時間軸細節。
